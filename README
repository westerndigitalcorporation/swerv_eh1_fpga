...
governed by the Apache-2.0 license (See LICENSE).  
Files under the common software (software/common) directory may be
...
How to build swerv_eh1 based reference design and run applications on Nexys4DDR board?
...
3. Configure swerv_eh1 core for the Nexys4DDR board. We use default
...
9. Next, we need to an application to run on this system. Go to software/apps folder and build the application using `make` command. 
We provide a makefile to generate the executable (e.g., hello.elf).
...
10. Once we generate an application executable, we need to configure openocd+GDB and UART device.
	...
	b. UART: we can see the uart ouptut using minicom
           ...
           $ dmesg | grep ttyUSB
	   ...
	   [19023.576527] usb 3-6: FTDI USB Serial Device converter
